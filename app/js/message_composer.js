/*!
 * Webogram v1.1.2 - messaging web application for MTProto
...
MessageComposer.templates = [
    ...
    {key: '@operational', value: 'Well, we are operating only in Bangalore as of now and shortly expanding in all the major cities in India.'},
    ...
    {key: '@inappropriate', value: 'I\’m sorry, we would not be the right company to help you with your question. We don\’t deal in…We appreciate your contact. Have a good day.'},
    {key: '@payment_preference', value: 'Would you prefer paying online or COD?'},
    {key: '@link', value: 'here is the payment link, kindly ping us once you make the payment for the further confirmation.'},
    {key: '@welcomeback', value: 'Hey, Welcome back! what would you like to order today?'},
    {key: '@anything', value: 'you can order anything from anywhere, we can get it delivered.'},
    {key: '@address', value: 'No. 167, 5th Block, 17th C Main Road, KHB Colony , Koramangala, Bangalore, Karnataka 560095'},
    {key: '@business', value: 'please write at business@instanoapp.com for all your queries'},
    {key: '@thankyou', value: 'Thank you for using Instano, have a good one! :)'},
    {key: '@sorry', value: 'sorry for the inconvenience'},
    {key: '@moment', value: 'please give me a moment while I check this information for you.'},
    {key: '@pickup', value: 'your order is picked up and should reach your place in a while.'},
    {key: '@spread', value: 'spread the word.. :blush::+1:'},
    {key: '@delivery_time', value: 'your order is processed and will deliver it within 20-30mins approx'},
    {key: '@shareus', value: 'Thank you for using our service! please share your experience on twitter bit.ly/instano0 / Facebook page www.fb.com/instanoapp'},
    {key: '@feedback', value: 'Thank you for using our service! Your feedback is highly appreciated and will help us to improve our ability to serve you better.'},
    {key: '@rateus', value: 'Happy with our services? Go ahead and rate our app bit.ly/instano on Google playstore!'},
    {key: '@referus', value: 'Here\'s the good news for you.. Refer a friend and get a FREE delivery!'},
    {key: '@pricing', value: 'We charge a minimum a conveniece fee of ₹30 and charges may vary based on your order, delivery location. '},
    {key: '@pricing_services', value: 'There is minimal charge of ₹150 as visiting/convenience fee + the amount quoted by the servicemen after visual inspection.'},
    {key: '@location', value: 'Please share us your delivery location'},
    {key: '@location_P&D', value: 'Please share us your pick up location & delivery location'},
    {key: '@sams_menu', value: 'You can order rice,noodles,rolls, partha\'s+curry, starters, Sandwich, raitha\'s, beverages,deserts.'},
    {key: '@niteout_menu', value: 'You can order rice,noodles,deserts, beverages, roti\'s+curry, snacks, beverages.'},
    {key: '@night_restaurant', value: 'We can get it deliverd from Nightpanda, Niteout, Samskitchen'},
    {key: '@night_delivery_time', value: 'We can get it delivered in less than 90 minutes.'},
    {key: '@hold_order_cofirmation', value: 'One moment pls, checking for the availability of the runner.'},
    {key: '@regret_pending', value: 'As we have too many orders pending, we are unable to process your request today. Sorry for the inconvenience!'}
];
...
MessageComposer.prototype.onKeyEvent = function (e) {
  ...
  if (e.type == 'keydown') {
    ...
    if (checkSubmit && e.keyCode == 13) { // keyCode '13' = enter
      var submit = false;
...
MessageComposer.prototype.checkAutocomplete = function (forceFull) {
  ...
  if (matches) {
    ...
    if (matches[2] == '@') { // mentions or templates
      if (this.mentions && this.mentions.index) { // mentions
        if (query.length) {
        ...
        if (query.length) {
          ...
          for (var i = 0; i < MessageComposer.templates.length; i++) {
            if(MessageComposer.templates[i].key.indexOf(query) > -1) // template contains query
              foundTemplates.push(MessageComposer.templates[i]);
          }
          this.showTemplateSuggestions(foundTemplates);
        } else {
          this.showTemplateSuggestions(MessageComposer.templates); // show all
        }
      }
    }
...
/**
 * basically pirated from MessageComposer.prototype.showCommandsSuggestions with some cosmetic changes
 */
MessageComposer.prototype.showTemplateSuggestions = function (templates) {
  ...
  for (i = 0; i < count; i++) {
    ...
    html.push('<li><a class="composer_command_option" data-template="' + encodeEntities(template.value) + '"><span class="composer_command_value">' + encodeEntities(template.key) + '</span><span class="composer_command_desc">' + encodeEntities(template.value) + '</span></a></li>');
  }
